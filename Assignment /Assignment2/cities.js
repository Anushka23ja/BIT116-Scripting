...
/* 	INSTRUCTIONS: Part 1
	...
	(2) Initialize the array elements with these city names (in UPPERCASE letters):
		...
		WOODINVILLE
*/
var city = ["BELLEVUE", "BOTHELL", "DUVALL", "EDMONDS",	"KENMORE", "KIRKLAND", "LAKE FOREST PARK",
...
function guessIt()
{
	...
	*/
	...
	var guess = document.getElementById('valuetwo').value;
	tries++;
	guessStatus();
	...
	(5) The default
		a) Change the hint value to "No more hints available! Last chance to guess!";
		   HINT: Look in existing code to see how this might be done
	*/
	valuethree = document.getElementById('valueone');
	switch (tries) {
        case 1:
			valuethree.value = "First Hint: The city name starts with \'" + cityName.charAt(0) + "\'";
            break;
        case 2:
			valuethree.value = "Second Hint: The city name ends with \'" + cityName.charAt(cityName.length - 1) + "\'";
            break;
		case 3:
			valuethree.value = "Last Hint: The city name has " + cityName.length + " characters";
			break;
        default:
            valuethree.value = "No more hints available! Last chance to guess!";
    }
